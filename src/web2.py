...
from src.accelerator import CoordinateTransformer, DividedParetoFront, GeneWindowGenerator
...
WINDOW_WIDTH = 20  # Width of the minimun window size in RESIZED_IMAGE_DIM

def get_pareto_front(pred_mask: np.ndarray, width_ratio: int, height_ratio: int) -> IParetoFront:
    """Process the predicted mask to get the best crop window."""
    pareto_front = ParetoFront(solution_dimensions=3)
    geneWindowGenerator = GeneWindowGenerator(pred_mask, width_ratio, height_ratio)

    for window in geneWindowGenerator.generate_windows():
        ...
        geneWindowGenerator.population = [solution.get_metadata() for solution in pareto_front.get_pareto_solutions()]
...
def process_image(image, prompts, crop_width_ratio, crop_height_ratio, use_owlv2):
    try:
        ...
        # Get the metadata based on the pareto front.
        resized_width_ratio, resized_height_ratio = coordinate_transformer.convert_original_ratio_to_resized(
            crop_width_ratio, crop_height_ratio, WINDOW_WIDTH
        )
        pareto_front = get_pareto_front(pred_mask, resized_width_ratio, resized_height_ratio)
